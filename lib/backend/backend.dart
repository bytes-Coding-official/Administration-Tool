...
import 'schema/customer_meeting_record.dart';
...
export 'schema/customer_meeting_record.dart';
...
/// Functions to query CustomerMeetingRecords (as a Stream and as a Future).
Future<int> queryCustomerMeetingRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
...
Stream<List<CustomerMeetingRecord>> queryCustomerMeetingRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
    queryCollection(
      CustomerMeetingRecord.collection,
      CustomerMeetingRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<CustomerMeetingRecord>> queryCustomerMeetingRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future<FFFirestorePage<CustomerMeetingRecord>> queryCustomerMeetingRecordPage({
  Query Function(Query)? queryBuilder,
  DocumentSnapshot? nextPageMarker,
  required int pageSize,
  required bool isStream,
  required PagingController<DocumentSnapshot?, CustomerMeetingRecord>
      controller,
  List<StreamSubscription?>? streamSubscriptions,
}) =>
    queryCollectionPage(
      ...
      if (isStream) {
        final streamSubscription =
            (page.dataStream)?.listen((List<CustomerMeetingRecord> data) {
          for (var item in data) {
            final itemIndexes = controller.itemList!
                .asMap()
                .map((k, v) => MapEntry(v.reference.id, k));
